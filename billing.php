<?php
    ...
    if(isset($_POST['name'])){
        ...
        $query = mysqli_query($link,"insert into pass(name, email, contact, date, dest, password) values ('$name', '$email', '$contact', '$date', '$dest', '$password')");
		$lid = mysqli_insert_id($link);
		
		$nod = round((strtotime($date) - time())/(60*60*24))+1;
		$result = mysqli_query($link,"select price from destination where name = '$dest'");
    }
?>
